import nodemailer from 'nodemailer';

/**
 * Adapter (Structural): `MailProvider` is the interface the app depends on.
 * `NodemailerAdapter` adapts the third-party nodemailer transport to that
 * interface, so the rest of the code never talks to nodemailer directly.
 */
export interface MailMessage {
  to: string;
  subject: string;
  text: string;
  html: string;
}

export interface MailProvider {
  send(message: MailMessage): Promise<unknown>;
}

export class NodemailerAdapter implements MailProvider {
  private readonly from =
    process.env.SMTP_FROM || '"Kanban Support" <noreply@kanban.local>';

  private createTransporter() {
    return nodemailer.createTransport({
      host: process.env.SMTP_HOST || 'smtp.ethereal.email',
      port: parseInt(process.env.SMTP_PORT || '587', 10),
      secure: process.env.SMTP_SECURE === 'true', // true for 465, false for other ports
      auth: {
        user: process.env.SMTP_USER, // generated ethereal user
        pass: process.env.SMTP_PASS, // generated ethereal password
      },
    });
  }

  async send(message: MailMessage): Promise<unknown> {
    const transporter = this.createTransporter();

    try {
      const info = await transporter.sendMail({ from: this.from, ...message });
      console.log('Message sent: %s', info.messageId);
      // For Ethereal, you can get a preview URL like this:
      // console.log('Preview URL: %s', nodemailer.getTestMessageUrl(info));
      return info;
    } catch (error) {
      console.error('Error sending email:', error);
      throw new Error('Failed to send verification email');
    }
  }
}

const mailProvider: MailProvider = new NodemailerAdapter();

export const sendPasswordResetEmail = async (to: string, code: string) => {
  return mailProvider.send({
    to,
    subject: 'Password Reset Verification Code',
    text: `Your password reset verification code is: ${code}. It will expire in 15 minutes.`,
    html: `<p>Your password reset verification code is: <strong>${code}</strong></p><p>It will expire in 15 minutes.</p>`,
  });
};
